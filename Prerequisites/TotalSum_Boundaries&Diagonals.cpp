...
int totalSum(int **input, int n) {
    ...
    int sum = getFirstDiagonalSum(input, n);
    
    sum += getSecondDiagonalSum(input, n);
    sum += getBoundarySum(input, n); 
    if (n % 2 != 0) {
        
        sum -= input[n / 2][n / 2];
        //To avoid double counting of middle element in case of odd 'n'
